...
| FlowPool thật | Bản sửa [FLOW-004](../logs/issues/FLOW-004.md) qua `doctor` thật: bốn profile `logged_in=true`, `flow_reachable=true`, số dư UI 1050/331/1050/1050. Lượt bootstrap mascot đầu tiên dừng `MODEL_NOT_SELECTABLE` **trước submit** ở hai profile; xem [FLOW-005](../logs/issues/FLOW-005.md). Bản sửa settings đã hợp nhất, daemon mới đang chờ Chrome Allow | Đăng nhập, dự án và số dư đã được xác minh; **chưa** có ảnh/clip thật, kiểm tham chiếu hay giá khấu trừ. Sau Allow, thử lại đúng cùng ID/journal, không tạo request khác. |
...
- Bốn profile đã qua `doctor` thật sau bản sửa nhận diện/số dư [FLOW-004](../logs/issues/FLOW-004.md). Daemon hiện nạp bản sửa chọn cài đặt và đang chờ Allow cho **lần kết nối mới này**; giữ đúng daemon, không mở client CDP khác. CAPTCHA/đăng xuất thật do người dùng xử lý.
- Lượt ảnh bootstrap cùng ID dừng trước submit khi UI còn ở Video; bản sửa chọn Image/model/9:16 đã hợp nhất. Sau Allow, thử lại đúng yêu cầu đã `not_submitted`; nếu UI tiếp tục lệch thì dừng với [FLOW-005](../logs/issues/FLOW-005.md), không đổi ID/prompt để né journal.
