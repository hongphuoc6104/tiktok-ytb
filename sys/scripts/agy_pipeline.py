...
def generate(p,job):
 ...
 try:
  ...
  if version:
   ...
   prompt+='\nViết đầy đủ content-v3, giữ nguyên outline. Mỗi cảnh có nhiều images/beats khi có lý do; overlays của beat (label/chapter_title/map_pin/counter/arrow) do renderer vẽ bằng tiếng Việt có dấu, ưu tiên hơn chữ trong ảnh; images kind "clip" cần from_image (ảnh still cùng cảnh) và motion tiếng Anh, không visible_text; được tái sử dụng ảnh. based_on chỉ ảnh trước trong cùng cảnh. Mỗi nhịp neo vào nguyên văn lời dẫn và lần xuất hiện; nhịp đầu neo đầu câu đầu; riêng vi/en. visible_text là danh sách chữ duy nhất AI được vẽ; không ghi mã nhân vật/cảnh/ảnh trong mô tả nhìn thấy. Chữ tạo cùng hình. Không bịa đã đo thời lượng. claims trích phát biểu và dữ kiện nguyên văn từ nguồn. Phản hồi sửa phải có revision_response, nêu rõ unresolved; không tự nhận đã được duyệt.'
