...
test('explicit local profile wins but is validated',()=>{
 const selected=browserConfig({flow_profile_directory:'Default'},{flow_user_data_dir:'/tmp/local-fixture',flow_profile_directory:'Profile 3'});
 assert.equal(selected.expectedProfilePath,'/tmp/local-fixture/Profile 3');
 assert.throws(()=>browserConfig({}, {flow_profile_directory:'../escape'}));
});
